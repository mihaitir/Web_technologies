...
public class ClassroomControllers {
	...
	public ResponseEntity<List<ClassStud>> getAllClassTeachByClassroom(@PathVariable Integer idClassroom) {
		return new ResponseEntity<List<ClassStud>>(this.iClassStudRepository.findClassStudByIdClassroom(idClassroom),
				HttpStatus.OK);
	}
	...
	public ResponseEntity<List<Student>> getStudentsByIdClassroom(@PathVariable Integer idClassroom) {
		...
		// and for every entity from that, we get the Student by his id, and push them
		// into a list
		List<Student> l = new ArrayList<Student>();

		ArrayList<ClassStud> list = (ArrayList<ClassStud>) this.iClassStudRepository
				.findClassStudByIdClassroom(idClassroom);
	...
	public ResponseEntity<StudTest> getTestResult(@PathVariable Integer idStudent, @PathVariable Integer idTest) {
		...
		if (this.iStudTestRepository.findByIdStudentAndIdTest(idStudent, idTest).isPresent())
			return new ResponseEntity<StudTest>(
					this.iStudTestRepository.findByIdStudentAndIdTest(idStudent, idTest).get(), HttpStatus.OK);
		else {
	...
	@GetMapping("classroom/getTestsResult2/{idStudent}/{idClassroom}")
	public ResponseEntity<List<StudTest>> getTestsResultByIdStudentAndIdClassroom(@PathVariable Integer idStudent,
			@PathVariable Integer idClassroom) {
		return new ResponseEntity<List<StudTest>>(
				this.iStudTestRepository.findByIdStudentAndIdClassroom(idStudent, idClassroom).get(), HttpStatus.OK);
	}
